	...
	for(int j=0; j<ITER_COUNT; j++) {
		for(int i=0; i<N; i++) {
			...
			if(heap.size()>1) {
				heap.remove(temp);
				x^=temp.data();
				//printf("haha %016lx %016lx\n",x, temp.data());
				if(temp.data()%16==0) Tracer::I()->meet(temp.key());
			}
			if(x%2==0&&heap.size()<N) {
				heap.insert(arr[i].b^x,arr[i].a^x);
				heap.insert(arr[i].a^x,arr[i].b^x);
			}
		}
	}
	delete[] arr;
...
#ifdef SELF_TEST
int main() {
	char hello[100]="ae90--i888f--r39invqbj43to;5j46354q34534999!@#%@#$%^&$&ADGSGWREF";
